#!/usr/bin/env python3
"""
DealLedger Discovery Module v2
================================
...
Priority order (highest to lowest):
  ...
  9. Playwright XHR  — network interception for JS-rendered + failed sites
 10. Flag            — js_only (Playwright unavailable), auth_required, dead

Run:
  python3 discovery_v2.py --url https://example.com
  python3 discovery_v2.py --batch data/brokers_clean.csv --workers 5
  python3 discovery_v2.py --batch data/brokers_clean.csv --rediscover
  python3 discovery_v2.py --batch data/brokers_clean.csv --js-only   # re-run js_only domains only
  python3 discovery_v2.py --print-sql
"""
...
def discover(url, verbose=True):
    ...
    if is_js_rendered(html):
        log(f"   ⚠️  JS-rendered — trying Playwright network interception...")
        if HAS_PLAYWRIGHT:
            pw_result = try_playwright_network(url)
            if pw_result:
                log(f"   ✅ Playwright found: {pw_result['endpoint']} ({pw_result['sample_count']} items)")
                pw_result.update({"platform": platform, "domain": domain, "url": url})
                cache_set_discovery(domain, url, pw_result)
                return pw_result
            log(f"   ⚠️  Playwright found no JSON endpoint — marking js_only")
        else:
            log(f"   ⚠️  Playwright not installed (pip install playwright && playwright install chromium)")
    ...
    # ── Step 13: Playwright fallback (last resort before failed) ─
    if HAS_PLAYWRIGHT:
        log(f"   🎭 Trying Playwright network interception (last resort)...")
        pw_result = try_playwright_network(url)
        if pw_result:
            log(f"   ✅ Playwright found: {pw_result['endpoint']} ({pw_result['sample_count']} items)")
            pw_result.update({"platform": platform, "domain": domain, "url": url})
...
_PW_CONTAINER_KEYS = ["items","results","data","listings","businesses","records","posts","products"]
...
def _pw_find_best_list(obj):
    ...
    def walk(node):
        ...
        if isinstance(node, dict):
            for k in _PW_CONTAINER_KEYS:
                s = _pw_score_list(node.get(k))
                if s > best_score: best_score, best = s, node[k]
            for v in node.values(): walk(v)
        elif isinstance(node, list):
            s = _pw_score_list(node)
            if s > best_score: best_score, best = s, node
            for it in node[:20]: walk(it)
    walk(obj)
    return best, best_score

def _pw_guess_pagination(url):
    q = {k.lower(): v for k, v in parse_qsl(urlsplit(url).query, keep_blank_values=True)}
    if any(k in q for k in ["cursor","nextcursor","after","pagecursor"]): return "cursor"
    if any(k in q for k in ["page","p","pg"]) and any(k in q for k in ["per_page","limit","pagesize","page_size"]): return "page_param"
    if any(k in q for k in ["offset","skip","start"]) and any(k in q for k in ["limit","take","count"]): return "offset"
    return "unknown"

def _pw_strip_tracking(url):
    parts = urlsplit(url)
    q = [(k,v) for k,v in parse_qsl(parts.query, keep_blank_values=True) if k.lower() not in _PW_TRACKING]
    return urlunsplit((parts.scheme, parts.netloc, parts.path, urlencode(q, doseq=True), ""))

def try_playwright_network(url, timeout_ms=25000):
    """
    Open page in headless Playwright, intercept XHR/fetch, trigger pagination
    interactions, return best ExtractionPlan dict or None.
    Called only when is_js_rendered() is True.
    """
    if not HAS_PLAYWRIGHT: return None

    candidates = {}

    def add_candidate(req, resp, body_text, parsed):
        k = _pw_strip_tracking(req.url)
        e = candidates.setdefault(k, {
            "url": req.url, "method": req.method,
            "headers": dict(req.headers),
            "hits": 0, "last_status": resp.status,
            "max_bytes": 0, "best_score": 0,
            "best_count": 0, "best_keys": [], "sample_url": req.url,
        })
        e["hits"] += 1
        e["last_status"] = resp.status
        e["max_bytes"] = max(e["max_bytes"], len(body_text or ""))
        best_list, score = _pw_find_best_list(parsed)
        if score > e["best_score"]:
            e["best_score"] = score
            e["best_count"] = len(best_list) if best_list else 0
            if best_list and isinstance(best_list[0], dict):
                e["best_keys"] = sorted(best_list[0].keys())[:40]
            e["sample_url"] = req.url
    ...
    def on_response(resp):
        try:
            ...
            add_candidate(req, resp, body, parsed)
        except: pass

    def try_interactions(page):
        page.wait_for_load_state("networkidle", timeout=timeout_ms)
        # Try next-page clicks
        for sel in ['a[rel="next"]','button[aria-label*="Next" i]','a[aria-label*="Next" i]',
                    'button:has-text("Next")','a:has-text("Next")','button:has-text("›")']:
            try:
        ...
        # Try sort dropdown
        for sel in ['select[name*="sort" i]','button:has-text("Sort")']:
            try:
                el = page.locator(sel).first
                if el.count() > 0 and el.is_visible():
    ...
    ranked = sorted(candidates.values(),
                    key=lambda x: (x["best_score"], x["best_count"], x["max_bytes"]),
                    reverse=True)
    winner = ranked[0]
    if winner["best_score"] < 30 or winner["best_count"] < 2: return None

    # Parse endpoint path from winner URL
    parsed_endpoint = urlsplit(winner["sample_url"])
    endpoint_path = parsed_endpoint.path
    if parsed_endpoint.query:
        endpoint_path += "?" + parsed_endpoint.query

    return {
        "method": "network_json",
        "endpoint": endpoint_path,
        "endpoint_url": winner["sample_url"],
        "pagination": _pw_guess_pagination(winner["sample_url"]),
        "request_method": winner["method"],
        "request_headers": {k:v for k,v in winner["headers"].items()
                           if k.lower() in {"accept","authorization","x-api-key","cookie","referer"}},
        "sample_count": winner["best_count"],
        "sample_keys": winner["best_keys"],
        "confidence": "playwright",
        "status": "ok",
    }
...
def run_batch(csv_path, workers=5, rediscover=False, js_only_mode=False):
    # Load URLs
    urls = []
    ...
    sb = get_supabase()
    if not rediscover and sb:
        try:
            cached = sb.table("broker_discovery").select("domain, status").execute()
            if js_only_mode:
                # Only re-run domains currently marked js_only
                js_domains = {r["domain"] for r in cached.data if r.get("status") == "js_only"}
                before = len(urls)
                urls = [u for u in urls if urlparse(u).netloc.lower().replace("www.","") in js_domains]
                print(f"   --js-only mode: targeting {len(urls)} js_only domains (of {before} total)")
            else:
...
CREATE TABLE IF NOT EXISTS broker_discovery (
    ...
    method         TEXT,    -- wordpress_api, shopify_api, squarespace_json, wix_api,
                            ...
                            -- haiku_css, network_json, js_only, auth_required, dead, failed
    endpoint       TEXT,    -- API path if applicable
    endpoint_url   TEXT,    -- full URL (network_json method)
    pagination     TEXT,    -- page_param, cursor, offset, next_url, none, rsc, unknown
    page_size      INT,
    ...
    request_method TEXT,    -- GET or POST (network_json)
    request_headers JSONB,  -- headers needed to replay request (network_json)
    confidence     TEXT,
    sample_count   INT,
    sample_keys    JSONB,   -- field names found in sample listing (network_json)
...
-- ALTER TABLE broker_discovery ADD COLUMN IF NOT EXISTS endpoint_url TEXT;
-- ALTER TABLE broker_discovery ADD COLUMN IF NOT EXISTS request_method TEXT;
-- ALTER TABLE broker_discovery ADD COLUMN IF NOT EXISTS request_headers JSONB;
-- ALTER TABLE broker_discovery ADD COLUMN IF NOT EXISTS sample_keys JSONB;
"""

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="DealLedger Discovery Module v2")
    ...
    parser.add_argument("--js-only", action="store_true",
                        help="Re-run only domains currently marked js_only (uses Playwright)")
    ...
    if args.batch:
        run_batch(args.batch, workers=args.workers, rediscover=args.rediscover,
                  js_only_mode=getattr(args, "js_only", False))
    elif args.url:
