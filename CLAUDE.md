...
### Main scrapers
- `scrapers/legacy_source.py` — correct all-states collector (use this, not older variants)
- `unified_broker_scraper_v2.py` — ML-based broker scraper with pattern detection
- `legacy_source.py` — orchestration pipeline (has known bugs, see below)
- `legacy_source.py` — BizQuest view count refresh (unresolved: needs `save_to_supabase()` method)

### GitHub Actions
- `bbs_daily.yml` — calls `legacy_source.py` with `SUPABASE_URL` and `SUPABASE_SERVICE_KEY` secrets
- Cron: `0 14 * * *` (9am ET) — if you see timezone drift, this is why

### Known bugs (fix before extending)
1. **`legacy_source.py` line 171** — NoneType crash during Supabase loader phase. Guard with null check before accessing listing fields.
2. **Cron timezone mismatch** — proposed fix already applied (`0 14 * * *`), verify if still drifting.
3. **`legacy_source.py`** — missing `save_to_supabase()` method; original working file is `legacy_source.py`.
...
- Do NOT use older scraper variants — `legacy_source.py` in `scrapers/` is canonical
